...
def s_to_T(cell_number, steps, Ts=np.linspace(1e-100, 10, 100), avg=lambda x: avg(x, 100)):
    ...
    res = []
    for T in Ts:
        this_res = MC_Heisenberg(T, cell_number, steps)
        if T in [Ts[i] for i in range(Ts.shape[0])]:
            np.savetxt(f'./res_4/res_4_at_T={T}', [r['H'] for r in this_res])
        res.append(this_res)
...
if __name__ == '__main__':
    ...
    np.savetxt('Ts_4', Ts_4)
    np.savetxt('res_4', res_4['last_s'])
