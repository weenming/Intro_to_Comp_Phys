...
def basis_wids(ws):
    ...
    for w in ws:
        ...
        psis.append(lambda x: np.sqrt(w / np.pi) *
                    np.exp(- w * (x - center) ** 2))
    return psis
...
def plot_wave_func(states, E, fpath_and_fname=f'tmp', plot_abs=False, c=lambda x: 'orange', label=None):
    ...
    for i in range(len(states)):
        if label is None:
            l = f'{i+1}-th wave function with energy {E[i] / np.sqrt(2)}' + \
                '$\cdot\sqrt{2}$'
            order = i
        elif label == 0:
            l = None
            ...
            l = f'{label(i)}-th wave function with energy {E[i] / np.sqrt(2)}' + \
                '$\cdot\sqrt{2}$'
            order = label(i)
        if plot_abs:
            ax.plot(x, np.power(states[i](x), 2),
                    label=l, color=c(i), zorder=order)
            ax.set_ylabel('$|\psi|$')
        else:
...
def main_width_dif_basis_num(basis_num, is_odd, v=V1):
    ...
    eigen_states_normalized = [get_eigenstates(
        C_normalized, basis, j) for j in range(len(basis))]

    # sort energy and states by E (sorted in ascending order)
    eigen_states_normalized = [x for _, x in sorted(
        zip(E, eigen_states_normalized), reverse=False)]
    E.sort()
    ...
    print([inner_product(eigen_states_normalized[i], eigen_states_normalized[i])
          for i in range(len(basis))])
...
def main_center_dif_basis_num_even_and_odd(basis_num, v=V1):
    basis = [basis_pos(center, odd=False)
             for center in np.linspace(-5, 5, basis_num)]
    basis += [basis_pos(center, odd=True)
              for center in np.linspace(-5, 5, basis_num)]
    H = calc_hamilton(basis, v=v)
    S = calc_s(basis)
    # eigenvalue problem: H'C' = EC', where H' = S^{-1/2}HS^{1/2}, C' = S^{1/2}C
...
def different_basis_width_pot_1(fpath='./result/V1/different_width_odd_and_even/'):
    ...
    for basis_num in [3, 4, 5, 6, 7, 8, 9, 10]:
        this_E_even, this_eigenstates_even = main_width_dif_basis_num(
            basis_num, is_odd=False)
        # np.savetxt(f'energy_basis_numeber{basis_num}.txt', this_E)
        ax.scatter([basis_num], this_E_even[0] / np.sqrt(2), color=cmap((np.log(
            np.abs(this_E_even[0] / np.sqrt(2) - 0.5)) + 14) / 20), marker='.', zorder=10)
        ax.scatter([basis_num], this_E_even[1] / np.sqrt(2), color=cmap((np.log(
            np.abs(this_E_even[1] / np.sqrt(2) - 2.5)) + 14) / 20), marker='x', zorder=10)
        ax.scatter([basis_num], this_E_even[2] / np.sqrt(2), color=cmap((np.log(
            np.abs(this_E_even[2] / np.sqrt(2) - 4.5)) + 14) / 20), marker='+', zorder=10)
        np.savetxt(fpath+f'basis_num{basis_num}_even.txt', this_E_even)

        this_E_odd, this_eigenstates_odd = main_width_dif_basis_num(
            basis_num, is_odd=True)
        # np.savetxt(f'energy_basis_numeber{basis_num}.txt', this_E)
        ax.scatter([basis_num], this_E_odd[0] / np.sqrt(2), color=cmap(
            (np.log(np.abs(this_E_odd[0] / np.sqrt(2) - 1.5)) + 14) / 20), marker='.', zorder=10)
        ax.scatter([basis_num], this_E_odd[1] / np.sqrt(2), color=cmap(
            (np.log(np.abs(this_E_odd[1] / np.sqrt(2) - 3.5)) + 14) / 20), marker='x', zorder=10)
        ax.scatter([basis_num], this_E_odd[2] / np.sqrt(2), color=cmap(
            (np.log(np.abs(this_E_odd[2] / np.sqrt(2) - 5.5)) + 14) / 20), marker='+', zorder=10)
        np.savetxt(fpath+f'basis_num{basis_num}_odd.txt', this_E_odd)

        plot_wave_func(this_eigenstates_even[:2]+this_eigenstates_odd[:2], np.append(this_E_even[:2], this_E_odd[:2]), fpath+f'wavefuncplot_basis_num={basis_num}.png', plot_abs=False, c=lambda x: plt.get_cmap(
            'Oranges')(- x * 60 + 150) if x < 2 else plt.get_cmap('Blues')(- x * 60 + 270), label=lambda x: [1, 3, 2, 4][x])
    # add color bar
    divider = make_axes_locatable(ax)
    ax_cb = divider.new_horizontal(size="5%", pad=0.05)
    cb1 = mpl.colorbar.ColorbarBase(
        ax_cb, cmap=mpl.cm.viridis, orientation='vertical')
    cb1.set_ticklabels([8.3e-7, 4.5e-5, 2.5e-3, 13.e-1, 7.3, 70])
    cb1.set_label('$\ln(|E_i-E_i^{theory}|/\hbar\omega)$')
    fig.add_axes(ax_cb)

    ax.scatter([], [], color=cmap(0), marker='.',
               label=f'$E={0.5}\hbar\omega$')
    ax.scatter([], [], color=cmap(0), marker='x',
               label=f'$E={2.5}\hbar\omega$')
    ax.scatter([], [], color=cmap(0), marker='+',
               label=f'$E={4.5}\hbar\omega$')
    ax.scatter([], [], color=cmap(0), marker='.',
               label=f'$E={1.5}\hbar\omega$')
    ax.scatter([], [], color=cmap(0), marker='x',
               label=f'$E={3.5}\hbar\omega$')
    ax.scatter([], [], color=cmap(0), marker='+',
               label=f'$E={5.5}\hbar\omega$')
...
def different_basis_width_pot_1_only_even(fpath='./result/V1/different_width_even/'):
    ...
    for basis_num in [3, 4, 5, 6, 7, 8, 9, 10]:
        ...
        plot_wave_func(this_eigenstates_even[:3], this_E_even[:3], fpath +
                       f'wavefuncplot_basis_num={basis_num}.png', plot_abs=False, c=lambda x: plt.get_cmap('Oranges')(- x * 40 + 150), label=lambda x: x+1)
    # add color bar
    divider = make_axes_locatable(ax)
    ax_cb = divider.new_horizontal(size="5%", pad=0.05)
    cb1 = mpl.colorbar.ColorbarBase(
...
def different_basis_center_pot_1(fpath='./result/V1/different_center_even/'):
    ...
    i = 0
    for basis_num in basis_nums:
        this_E_even, this_eigenstates_even = main_center_dif_basis_num_even(
            np.linspace(-10, 10, basis_num), v=V1, w=3)
        # np.savetxt(f'energy_basis_numeber{basis_num}.txt', this_E)
        # this cmap normalizes to 1 (input \in (0, 1))
        cmap = plt.get_cmap('viridis')
        plot_wave_func(this_eigenstates_even[:4], this_E_even[:4], fpath +
                       f'wavefuncplot_basis_num={basis_num}.png', c=lambda x: plt.get_cmap('Oranges')(- x * 40 + 150))
        this_E_evens[i, :] = this_E_even[:4]
        i += 1
        np.savetxt(fpath+f'basis_num{basis_num}_even.txt', this_E_even)
    print(this_E_evens)

    ax.scatter(basis_nums, this_E_evens[:, 0] / np.sqrt(2), color=cmap((np.log(
        np.abs(this_E_evens[:, 0] / np.sqrt(2) - 0.5)) + 14) / 20), marker='.', zorder=10)
    ax.scatter(basis_nums, this_E_evens[:, 1] / np.sqrt(2), color=cmap((np.log(
        np.abs(this_E_evens[:, 1] / np.sqrt(2) - 1.5)) + 14) / 20), marker='x', zorder=10)
    ax.scatter(basis_nums, this_E_evens[:, 2] / np.sqrt(2), color=cmap((np.log(
        np.abs(this_E_evens[:, 2] / np.sqrt(2) - 2.5)) + 14) / 20), marker='+', zorder=10)
    ax.scatter(basis_nums, this_E_evens[:, 3] / np.sqrt(2), color=cmap((np.log(
        np.abs(this_E_evens[:, 3] / np.sqrt(2) - 3.5)) + 14) / 20), marker='v', zorder=10)
    ...
    ax.scatter([], [], color=cmap(0.5), marker='.',
               label=f'$E={0.5}\hbar\omega$')
    ax.scatter([], [], color=cmap(0.5), marker='x',
               label=f'$E={1.5}\hbar\omega$')
    ax.scatter([], [], color=cmap(0.5), marker='+',
               label=f'$E={2.5}\hbar\omega$')
    ax.scatter([], [], color=cmap(0.5), marker='v',
               label=f'$E={3.5}\hbar\omega$')
...
def different_basis_center_pot_2(fpath='./result/V2/different_center/'):
    ...
    for basis_num in basis_nums:
        this_E_even, this_eigenstates_even = main_center_dif_basis_num_even(
            np.linspace(-10, 10, basis_num), v=V2, w=3)
        # np.savetxt(f'energy_basis_numeber{basis_num}.txt', this_E)
        # this cmap normalizes to 1 (input \in (0, 1))
        cmap = plt.get_cmap('viridis')
        plot_wave_func(this_eigenstates_even[:4], this_E_even[:4], fpath +
...
def demo():
    E, eigen_states = main_center_dif_basis_num_even(
        np.linspace(-10, 10, 60), v=V3, w=3)
    print('eigenvalues of Hamiltonian are:')
    print(E.sort())
    plot_wave_func(eigen_states[:3], E[:3], plot_abs=True,
                   c=lambda x: plt.get_cmap('tab10')(x))
