...
def construct_matrix_CN(V, x, dt):
    x_grid = x.shape[0]
    dx = (x[-1] - x[0]) / x_grid
    ...
    for i in range(0, x_grid):
        ...
        if i < x_grid - 1:
            HD[i, i + 1] = - 1 / (2 * dx ** 2)
        HD[i, i] = 1 / dx ** 2 + V(x[i])
    I = np.identity(x_grid)
    # 抄 Slides 都能抄错，，，，，
    A = np.linalg.inv(I + 1j * dt * HD / 2) @ (I - 1j * dt * HD / 2)
    return A
...
def get_gaussian(x, x0, p):
    # omega = 2 / a ^ 2 so E = 1 / 2 hbar \omega = 1
    a = 1
...
if __name__ == '__main__':
    ...
    if input('re-calculate?') in ['y', 'Y']:
        sol, portions = solve(construct_matrix_CN, psi0, x, V, t_max, dt)
        np.savetxt('solution_CN.txt', sol)
        np.savetxt('portions_CN.txt', portions)

    sol = np.loadtxt('solution_CN.txt', dtype='complex')
    portions = np.loadtxt('portions_CN.txt', dtype='float')
    ...
    '''
    T^-1 = 1 + {V_0 ^ 2 \over 4 E (E + V_0)} \sin ({2a \over hbar} \sqrt{2m (E + V_0)})
     = 1 + 1 / 4 / 2 * sin(2 * sqrt{2 * 2}) ** 2
    '''
    fig, ax = plt.subplots(1, 1)
